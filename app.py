...
app = Flask(__name__)
model = pickle.load(open('Customer_Churn_Predictionkar.pkl', 'rb'))
@app.route('/', methods=['GET'])
def Home():
    return render_template('karnewindex.html')
...
def predict():
    if request.method == 'POST':
        ...
        if prediction==True:
             return render_template('karnewindex.html',prediction_text="The Customer will leave the bank")
        else:
             return render_template('karnewindex.html',prediction_text="The Customer will not leave the bank")
